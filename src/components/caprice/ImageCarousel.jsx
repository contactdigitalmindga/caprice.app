...
export default function ImageCarousel({images,alt}){
  ...
  if(images.length===1)return <Image src={images[0]} alt={alt} loading="eager" className="h-full w-full"/>;
  ...
  return <><div style={{position:'relative',height:'100%',width:'100%',overflow:'hidden'}}><div style={{display:'flex',height:'100%',transition:'transform .3s ease',transform:`translateX(-${active*100}%)`}}>{images.map((s,i)=><div key={i} style={{minWidth:'100%',height:'100%'}}><Image src={s} alt={`${alt} ${i+1}`} loading="eager" className="h-full w-full"/></div>)}</div>{active>0&&<button style={{...arrow,left:12}} onClick={()=>go(-1)}><ChevronLeft size={20}/></button>}{active<images.length-1&&<button style={{...arrow,right:12}} onClick={()=>go(1)}><ChevronRight size={20}/></button>}</div><div className="img-dots">{images.map((_,i)=><i key={i} className={i===active?'active':''}/>)}</div></>;
}