...
import { hero, heroSlides, mapMenuItem } from '@/lib/caprice';
...
export default function AppContext({children}){
  ...
  useEffect(()=>{const onboards=[hero,'https://media.base44.com/images/public/6a93174580eda443a786e6c3/7770644eb_generated_image.png','https://media.base44.com/images/public/6a93174580eda443a786e6c3/281f9526f_generated_image.png'];const staticP=Promise.all([hero,...onboards,...heroSlides].map(preloadImage));base44.functions.invoke('fetchCentralMenu').then(res=>{const p=(res.data?.items||[]).map(mapMenuItem);setProducts(p);Promise.all([preloadProducts(p.filter(v=>v.featured).slice(0,6)),staticP]).then(()=>setImagesReady(true))}).catch(()=>{setProducts([])}).finally(()=>setLoading(false))},[]);
  ...
  const refreshProducts=async()=>{const res=await base44.functions.invoke('fetchCentralMenu');const p=(res.data?.items||[]).map(mapMenuItem);setProducts(p);preloadProducts(p)};
  ...
  return <C.Provider value={{screen,setScreen,switchTab,tab,products,setProducts,refreshProducts,cart,setCart,revertCart,fav,toggle,toggleFavOptimistic,loadingFav,selected,open,add,loading,imagesReady,user,refreshUser,logout,goLogin,goRegister}}>{children}</C.Provider>;
}